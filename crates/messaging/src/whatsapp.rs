...
impl MessagingPlatform for WhatsAppAdapter {
    ...
    fn verify_signature(&self, payload: &[u8], signature: &str) -> Result<(), MessagingError> {
        let hex_sig = signature
            .strip_prefix("sha256=")
            .ok_or(MessagingError::InvalidSignature)?;
    ...
    fn parse_webhook(&self, payload: &[u8]) -> Result<Option<InboundMessage>, MessagingError> {
        let body: WaWebhook = serde_json::from_slice(payload)
            .map_err(|e| MessagingError::InvalidPayload(e.to_string()))?;
        let entry = match body.entry.first() {
            Some(e) => e,
            None => return Ok(None),
        };
        ...
        let v = &change.value;
        let msg = match v.messages.as_ref().and_then(|m| m.first()) {
            Some(m) => m,
            None => return Ok(None),
        };
        ...
        let is_group = v
            .metadata
            .as_ref()
            .and_then(|m| m.display_phone_number.as_ref())
            .is_some();
        let is_mention = text
            .as_ref()
            .map(|t| {
        ...
        let (quoted_id, quoted_text) = msg
            ...
            .unwrap_or((None, None));
        let ts = chrono::DateTime::from_timestamp(msg.timestamp.parse::<i64>().unwrap_or(0), 0)
        ...
        Ok(Some(InboundMessage {
            ...
            text,
            timestamp: ts,
            is_mention_to_bot: is_mention || !is_group,
    ...
    fn build_send_request(
        ...
        if let Some(ref r) = message.reply_to {
            body["context"] = serde_json::json!({"message_id":r});
        }
    ...
    fn handle_verification_challenge(
        ...
        let mode = params
            .get("hub.mode")
            .ok_or_else(|| MessagingError::VerificationFailed("missing hub.mode".into()))?;
        let token = params
            .get("hub.verify_token")
            .ok_or_else(|| MessagingError::VerificationFailed("missing token".into()))?;
        let challenge = params
            .get("hub.challenge")
            .ok_or_else(|| MessagingError::VerificationFailed("missing challenge".into()))?;
        if mode != "subscribe" || token != &self.verify_token {
            return Err(MessagingError::VerificationFailed("mismatch".into()));
        }
        Ok(challenge.clone())
    }
}
...
#[derive(Deserialize)]
pub struct WaValue {
    pub metadata: Option<WaMeta>,
    pub contacts: Option<Vec<WaContact>>,
    pub messages: Option<Vec<WaMsg>>,
}
#[derive(Deserialize)]
pub struct WaMeta {
...
mod tests {
    ...
    fn simple_payload(
        ...
        display_phone_number: Option<&str>,
    ) -> Vec<u8> {
        let meta = match (phone_number_id, display_phone_number) {
            (Some(pid), Some(dpn)) => {
                serde_json::json!({"phone_number_id": pid, "display_phone_number": dpn})
            }
