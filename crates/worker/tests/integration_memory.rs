...
fn create_get_update_delete_memory() {
    ...
    // 1. Create
    let resp = client
        .post(format!("{BASE}/api/w/{}/memory", slug()))
    ...
    // 2. Get
    let resp = client
        .get(format!("{BASE}/api/w/{}/memory/{id}", slug()))
        .header("authorization", &auth)
        .send()
        .unwrap();
    ...
    // 3. Update
    let resp = client
        .put(format!("{BASE}/api/w/{}/memory/{id}", slug()))
        .header("authorization", &auth)
    ...
    let resp = client
        .get(format!("{BASE}/api/w/{}/memory", slug()))
        .header("authorization", &auth)
        .send()
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    // 5. Delete
    let resp = client
        .delete(format!("{BASE}/api/w/{}/memory/{id}", slug()))
        .header("authorization", &auth)
        .send()
        .unwrap();
