...
fn conn() -> Connection {
    ...
    c.execute_batch(include_str!("../../../migrations/workspace/0003_calendar.sql"))
        .expect("apply 0003_calendar");
    c.execute_batch(include_str!("../../../migrations/workspace/0004_scheduling.sql"))
        .expect("apply 0004_scheduling");
    c
}

fn count(c: &Connection, sql: &str, params: &[&str]) -> i64 {
    c.query_row(sql, rusqlite::params_from_iter(params.iter()), |r| r.get(0)).unwrap()
}
...
fn todo_deadline_compared_as_civil_date() {
    ...
    c.execute("INSERT INTO todos (id, seq_num, title, deadline) VALUES ('t1', 1, 'civil', '2026-06-05')", []).unwrap();
...
fn event_all_day_and_timed_both_in_range() {
    let c = conn();
    c.execute("INSERT INTO events (id, title, starts_at) VALUES ('e1', 'allday', '2026-05-31')", []).unwrap();
    c.execute("INSERT INTO events (id, title, starts_at) VALUES ('e2', 'timed', '2026-05-31T18:00:00Z')", []).unwrap();
    ...
    assert_eq!(n, 2, "datetime() normalizes the bare date and the instant alike");
}
...
fn this_week_window_filters_by_bound() {
    ...
    assert_eq!(n, 1, "only the todo completed after the week-start bound counts");
}
...
fn timezone_seed_default_is_utc() {
    ...
    assert_eq!(tz, "UTC", "new workspaces default to UTC, not a locale-specific zone");
}
