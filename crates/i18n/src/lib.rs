...
impl Locale {
    ...
    pub fn from_accept_language(header: &str) -> Locale {
        for tag in header.split(',') {
            ...
            if l != Locale::En || tag.starts_with("en") {
                return l;
            }
        }
        Locale::En
...
mod tests {
    ...
    fn locale_round_trip() {
        for l in Locale::ALL {
            assert_eq!(*l, Locale::from_code(l.code()));
        }
    }
    #[test]
    fn accept_language() {
        assert_eq!(
            Locale::Fr,
            Locale::from_accept_language("fr-FR,fr;q=0.9,en;q=0.8")
        );
    ...
    #[test]
    fn unknown_key() {
        assert_eq!("__missing", t(Locale::En, "__missing", &[]));
    }
