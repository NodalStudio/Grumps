        ...
        >
            <div class="flex items-start gap-2">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-0.5 text-body-sm" style="color: var(--ink-40);">
                        <span class="capitalize font-medium">{atype.clone()}</span>
                        <span>"·"</span>
                        <span>{item.trigger_at.clone()}</span>
                        {item.recurrence.clone().map(|r| view! {
                            <span>"·"</span>
                            <span class="font-mono text-eyebrow">{r}</span>
                        })}
                    </div>
                </div>
                // Status badge
                <span
                    class="px-2 py-0.5 text-eyebrow rounded-sm flex-shrink-0"
                    style:background=move || status_color(&status)
                    style="color: white;"
                >{status2.clone()}</span>
            </div>

            <div class="text-body-sm" style="color: var(--ink-40);">
            ...
            <div class="flex items-center gap-2 pt-1 border-t" style="border-color: var(--ink-08);">
                ...
                >"Edit"</button>
                <button
                    class="text-body-sm font-semibold px-2 py-0.5 border rounded-sm cursor-pointer"
